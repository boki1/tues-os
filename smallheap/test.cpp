...
TEST(Smallheap, BasicAllocationTest) {
    smallheap heap;
    auto big1 = heap.allocate(smallheap::PAGE_SIZE << 1);
    auto a1 = heap.allocate(13).value();
    auto a2 = heap.allocate(50).value();
    auto big2 = heap.allocate(smallheap::PAGE_SIZE << 1);
...
TEST(Smallheap, AllocationMapTest) {
// FIXME:
#if 0
